...
class PointerNetwork(nn.Module):
    ...
    def forward(self, input):
        # [Tips] bs: batch_size; H: hidden_size; L: input_length; W: weight_size;
        # input:  (bs, L)
        batch_size = input.size(0)
        input = self.emb(input)  # (bs, L, embd_size)

        # Encoding
        encoder_states, hc = self.enc(input) # encoder_state: (bs, L, H),  hc: (1, bs, H)
        encoder_states = encoder_states.transpose(1, 0) # (L, bs, H)

        # Decoding states initialization
        decoder_input = to_var(torch.zeros(batch_size, self.emb_size)) # (bs, emb_size)
        hidden = to_var(torch.zeros([batch_size, self.hidden_size]))   # (bs, h)
        cell_state = encoder_states[-1]                                # (bs, h)  for LSTM cell

        probs = []
        # Decoding
        for i in range(self.answer_seq_len):  # range(M)
            # print(decoder_input)
            # decoder_input is zeros tensor for each time
            if self.is_GRU:
            ...
            blend1 = self.W1(encoder_states)           # (L, bs, W) = (L, bs, H) * (H, W)
            blend2 = self.W2(hidden)                   # (bs, W)    = (bs, H) * (H, W)
            blend_sum = torch.tanh(blend1 + blend2)    # (L, bs, W)
            out = self.vt(blend_sum).squeeze()         # (L, bs)    = (L, bs, W) * (W, 1)
